...
#
#   NAKAMA_DB_PASSWORD  NAKAMA_SERVER_KEY  NAKAMA_HTTP_KEY  NAKAMA_CONSOLE_PASSWORD
#
...
if grep -qE '@@(DB_PASSWORD|SERVER_KEY|HTTP_KEY|CONSOLE_PASSWORD)@@' "$out"; then
  echo "render-config: $out still has an unreplaced placeholder" >&2
  grep -nE '@@(DB_PASSWORD|SERVER_KEY|HTTP_KEY|CONSOLE_PASSWORD)@@' "$out" >&2
  rm -f "$out"
  exit 1
fi
