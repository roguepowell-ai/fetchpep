...
| D-070 | **The billing kill switch is written once by Claude Code, reviewed and merged by George, applied by operations with George signed in.** After it exists, no agent modifies it — the rule in `NON-NEGOTIABLES` and `.claude/rules/infra.md` applies from then on. Resolves the conflict between that rule and the first brief | George, 24 Sep — chose "Claude writes it once, I approve" | **Decided.** `.claude/rules/infra.md` wording updated to match; George places the file |
