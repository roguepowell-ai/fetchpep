...
| D-094 | **R2 is on hold** — no card on Cloudflare for now, sprites stay placeholders as in release-0001, and R2 is switched on when real art is ready | D-096 | Decided and superseded within hours on 25 Sep, before it was ever written into `state/DECISIONS.state.md`. D-096 ships creature art inside the app instead, so there is nothing to hold: R2 leaves the stack rather than waiting. Recorded here because "on hold" and "not in the stack" are different answers, and the first one was given |
