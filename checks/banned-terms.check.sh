...
# Use it only where naming the term IS the content — a decision log entry recording
# that the term was banned, for instance. Every exemption is visible in the diff.
set -euo pipefail
fail=0
...
while IFS= read -r term; do
  ...
  hits=$(grep -rniI --exclude-dir=.git --exclude-dir=node_modules \
